...
- **Chrome split.** The bottom pill is the only nav chrome at every width: fixed, centered, 12px from the bottom, max 440px wide (52px tall with row tabs at 960px and up). The former desktop top rail was retired in August 2026 so big screens keep the same thumb-first navigation, centered over the content column instead of stretched across the window. Wide screens at 960px and up add one quiet GitHub button in the page header, title left and source link right. It hides on phones.
